...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("rule_type", postgresql.ENUM("price_threshold", "percent_move", "candle_close", "macd_cross", name="rule_type_enum", create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("delivery_status", postgresql.ENUM("pending", "delivered", "failed", name="delivery_status_enum", create_type=False), server_default="pending", nullable=False),
    ...
    op.create_unique_constraint("uq_alert_window", "alerts", ["rule_id", "window_start", "window_end"])
    op.create_unique_constraint("uq_candle_symbol_time_interval", "candles", ["symbol", "timestamp", "interval_seconds"])
    ...
    op.create_index("idx_alerts_delivery_status", "alerts", ["delivery_status"], postgresql_where=sa.text("delivery_status = 'pending'"))
    op.create_index("idx_alerts_triggered_at", "alerts", [sa.text("triggered_at DESC")])
    op.create_index("idx_rules_active_symbol", "rules", ["is_active", "symbol"], postgresql_where=sa.text("is_active = true"))
    op.create_index("idx_candles_symbol_timestamp", "candles", ["symbol", sa.text("timestamp DESC")])
